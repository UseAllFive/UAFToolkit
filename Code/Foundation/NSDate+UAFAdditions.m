...
#import "NSDate+UAFAdditions.h"

#define ISO8601_MAX_LEN 25

@implementation NSDate (UAFAdditions)
...
- (NSString *)briefTimeInWords {
	...
	if (seconds < 60.0) {
		if (seconds < 2.0) {
			return UAFLocalizedString(@"1s");
		}
		return [NSString stringWithFormat:UAFLocalizedString(@"%ds"), (NSInteger)seconds];
	}
	...
	if (minutes >= 0.0 && minutes < 60.0) {
		if (minutes < 2.0) {
			return UAFLocalizedString(@"1m");
		}
		return [NSString stringWithFormat:UAFLocalizedString(@"%dm"), (NSInteger)minutes];
	}
	...
	// Hours
		...
		if (hours < 2) {
			return UAFLocalizedString(@"1h");
		}
		return [NSString stringWithFormat:UAFLocalizedString(@"%dh"), hours];
	}
	...
	// Days
		...
		if (days < 2) {
			return UAFLocalizedString(@"1d");
		}
		return [NSString stringWithFormat:UAFLocalizedString(@"%@d"),
	...
	// Years
		...
		if (years < 2) {
			return UAFLocalizedString(@"1y");
		}
		return [NSString stringWithFormat:UAFLocalizedString(@"%@y"),
...
+ (NSString *)timeInWordsFromTimeInterval:(NSTimeInterval)intervalInSeconds includingSeconds:(BOOL)includeSeconds {
	...
	if (intervalInMinutes >= 0 && intervalInMinutes <= 1) {
		if (!includeSeconds) {
			return intervalInMinutes <= 0 ? UAFLocalizedString(@"less than a minute") : UAFLocalizedString(@"1 minute");
		}
		if (intervalInSeconds >= 0 && intervalInSeconds < 5) {
			return [NSString stringWithFormat:UAFLocalizedString(@"less than %d seconds"), 5];
		} else if (intervalInSeconds >= 5 && intervalInSeconds < 10) {
			return [NSString stringWithFormat:UAFLocalizedString(@"less than %d seconds"), 10];
			...
			return UAFLocalizedString(@"half a minute");
		} else if (intervalInSeconds >= 40 && intervalInSeconds < 60) {
			return UAFLocalizedString(@"less than a minute");
	 	} else {
			return UAFLocalizedString(@"1 minute");
		}
	} else if (intervalInMinutes >= 2 && intervalInMinutes <= 44) {
		return [NSString stringWithFormat:UAFLocalizedString(@"%d minutes"), (NSInteger)intervalInMinutes];
	} else if (intervalInMinutes >= 45 && intervalInMinutes <= 89) {
		return UAFLocalizedString(@"about 1 hour");
	} else if (intervalInMinutes >= 90 && intervalInMinutes <= 1439) {
		return [NSString stringWithFormat:UAFLocalizedString(@"about %d hours"), (NSInteger)round(intervalInMinutes / 60.0f)];
	} else if (intervalInMinutes >= 1440 && intervalInMinutes <= 2879) {
		return UAFLocalizedString(@"1 day");
	} else if (intervalInMinutes >= 2880 && intervalInMinutes <= 43199) {
		return [NSString stringWithFormat:UAFLocalizedString(@"%d days"), (NSInteger)round(intervalInMinutes / 1440.0f)];
	} else if (intervalInMinutes >= 43200 && intervalInMinutes <= 86399) {
		return UAFLocalizedString(@"about 1 month");
	} else if (intervalInMinutes >= 86400 && intervalInMinutes <= 525599) {
		return [NSString stringWithFormat:UAFLocalizedString(@"%d months"), (NSInteger)round(intervalInMinutes / 43200.0f)];
	} else if (intervalInMinutes >= 525600 && intervalInMinutes <= 1051199) {
		return UAFLocalizedString(@"about 1 year");
	} else {
		return [NSString stringWithFormat:UAFLocalizedString(@"over %d years"), (NSInteger)round(intervalInMinutes / 525600.0f)];
	}
	return nil;
}
